...
import com.example.usermanagement.util.Config;
...
public class BigQueryUserListServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            if (dataset == null || dataset.isEmpty()) {
                dataset = Config.get("DEFAULT_BIGQUERY_DATASET", "user_dataset");
            }
            if (table == null || table.isEmpty()) {
                table = Config.get("DEFAULT_BIGQUERY_TABLE", "User");
            }
            ...
            List<UserRecord> allUsers = BigQueryUtil.listUsers(dataset, table);
            ...
            error.addProperty("error", "Query interrupted: " + e.getMessage());
            resp.getWriter().write(gson.toJson(error));
        } catch (Exception e) {
            resp.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            JsonObject error = new JsonObject();
            error.addProperty("error", "Failed to fetch users from BigQuery: " + e.getMessage());
            resp.getWriter().write(gson.toJson(error));
        }
    }
}
